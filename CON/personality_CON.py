...
modelleft.add(GlobalMaxPooling1D())
...
modellstm.add(Dense(1, activation='sigmoid',name='Dense_out')) # 5 responds to 5 personalities
...
#don't run this line
history=modellstm.fit([X_train,ma_train],y_train[:,3],validation_data=[[X_test,ma_test],y_test[:,3]],epochs=50, batch_size=200,callbacks=callbacks)

modellstm.save_weights('CON.h5')
# In[53]:
...
fig.savefig('learning_CON_50drop.png')
